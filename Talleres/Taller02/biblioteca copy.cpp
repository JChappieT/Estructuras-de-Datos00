...
class ColaOrdenada{
    ...
    public:
        ...
        void setColaPalabras(queue<string> colaPalabras){
           this->colaPalabras = colaPalabras;

        };

        queue<string> getColaParabras(){
            return this->colaPalabras;
        }
        ...
        void cargarArchivo(string a){
            ...
            archivo >> palabra; // Leer el numero de lineas, que no me sirve para nada, en el codigo que hicimos
            archivo >> this->subcadena;  // Leer la subcadena
            cout << this->subcadena << endl;
            while (getline(archivo, linea)) { // Leer línea por línea
                ...
                while (streamLinea >> palabra) { // Extraer palabras de la línea
                    this->colaPalabras.push(palabra);
                    if (this->colaPalabras.front().find(this->subcadena) != string::npos) {
                        cantidadP++; //Contador de numero de palabras que contienen la subcadena
                        cout << this->colaPalabras.front() << " Linea: " << this->numeroLinea+1 << endl;
                    }
                    this->colaPalabras.pop();
                }
            }
